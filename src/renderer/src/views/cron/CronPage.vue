...
const TARGET_MODEL = 'google-vertex/gemini-2.5-flash'
...
async function handleBatchSetModel() {
  ...
  try {
    const count = await cronStore.batchSetModel(TARGET_MODEL)
    message.success(t('pages.cron.batchSetModelSuccess', { count }))
  } catch {
    message.error(t('pages.cron.batchSetModelFailed'))
  } finally {
...
function formatCronAsCn(expr: string, tz?: string): string {
  ...
  const weekdayMap: Record<string, string> = {
    '0': t('pages.cron.schedule.weekdaySun'),
    '7': t('pages.cron.schedule.weekdaySun'),
    '1': t('pages.cron.schedule.weekdayMon'),
    '2': t('pages.cron.schedule.weekdayTue'),
    '3': t('pages.cron.schedule.weekdayWed'),
    '4': t('pages.cron.schedule.weekdayThu'),
    '5': t('pages.cron.schedule.weekdayFri'),
    '6': t('pages.cron.schedule.weekdaySat'),
    SUN: t('pages.cron.schedule.weekdaySun'),
    MON: t('pages.cron.schedule.weekdayMon'),
    TUE: t('pages.cron.schedule.weekdayTue'),
    WED: t('pages.cron.schedule.weekdayWed'),
    THU: t('pages.cron.schedule.weekdayThu'),
    FRI: t('pages.cron.schedule.weekdayFri'),
    SAT: t('pages.cron.schedule.weekdaySat'),
  }
  ...
  const parseDowPart = (value: string): string | null => {
    if (value.includes('-')) {
      ...
      return startText && endText ? t('pages.cron.schedule.weekdayRange', { start: startText, end: endText }) : null
    }
...
async function handleSelectJob(row: CronJob) {
  try {
    ...
    console.error('[CronPage] fetchRuns failed:', e)
    message.error(t('pages.cron.messages.fetchRunsFailed'))
  }
}
    ...
    <NCard class="cron-hero" :bordered="false">
      ...
      <template #header-extra>
        <NSpace :size="8">
          ...
          <NPopconfirm @positive-click="handleBatchSetModel">
            <template #trigger>
              <NButton size="small" type="warning" :loading="batchingModel">
                {{ t('pages.cron.batchSetModel') }}
              </NButton>
            </template>
            {{ t('pages.cron.batchSetModelConfirm', { model: TARGET_MODEL }) }}
          </NPopconfirm>

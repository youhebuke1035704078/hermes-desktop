...
import {
  ChatbubblesOutline, RefreshOutline, LinkOutline, UnlinkOutline,
  SettingsOutline, KeyOutline,
...
const columns = computed<DataTableColumns<Channel>>(() => [
  ...
  },
  {
    ...
    render(row) {
      return h('div', [
        h(NText, { strong: true, style: 'display: block;' }, {
          default: () => row.accountName || row.accountId || row.id,
        }),
        h(NText, { depth: 3, style: 'font-size: 11px; font-family: monospace;' }, {
          default: () => row.id,
        }),
      ])
    },
  },
  {
    ...
    width: 120,
    render(row) {
      return h(NTag, { size: 'small', bordered: false, type: statusType(row.status) }, {
        ...
        <NDrawerContent :title="channelsStore.selectedChannel?.id || ''">
          <template v-if="channelsStore.selectedChannel">
            <NSpace vertical :size="12">
              <div>
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.platform') }}</NText>
                <div>
              ...
              <div>
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.status') }}</NText>
                <div>
                  <NTag size="small" :bordered="false" :type="statusType(channelsStore.selectedChannel.status)">
                    {{ statusLabel(channelsStore.selectedChannel.status) }}
                  </NTag>
                </div>
              </div>
              <div v-if="channelsStore.selectedChannel.accountName">
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.accountName') }}</NText>
                <div><NText>{{ channelsStore.selectedChannel.accountName }}</NText></div>
              </div>
              <div v-if="channelsStore.selectedChannel.accountId">
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.accountId') }}</NText>
                <div><NText code style="font-size: 12px;">{{ channelsStore.selectedChannel.accountId }}</NText></div>
              </div>
              <div v-if="channelsStore.selectedChannel.memberCount != null">
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.memberCount') }}</NText>
                <div><NText>{{ channelsStore.selectedChannel.memberCount }}</NText></div>
              </div>
              <div>
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.dmPolicy') }}</NText>
                <div><NText>{{ channelsStore.selectedChannel.dmPolicy }}</NText></div>
              </div>
              <div v-if="channelsStore.selectedChannel.groupPolicy">
                <NText depth="3" style="font-size: 12px;">{{ t('pages.channels.detail.groupPolicy') }}</NText>
                <div><NText>{{ channelsStore.selectedChannel.groupPolicy }}</NText></div>
              </div>
              <div v-if="channelsStore.selectedChannel.groups && channelsStore.selectedChannel.groups.length > 0">
                <NText depth="3" style="font-size: 12px;">
                  {{ t('pages.channels.detail.groups', { count: channelsStore.selectedChannel.groups.length }) }}
                </NText>

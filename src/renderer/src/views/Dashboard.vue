...
let prevWsState = ''
let pendingRefreshOnConnect = false
let refreshedStartDate = ''
let refreshedEndDate = ''
...
const usageKpis = computed(() => {
  ...
  const cacheInputTotal = totals.input + totals.cacheRead
  const cacheReadRatio = cacheInputTotal > 0 ? (totals.cacheRead / cacheInputTotal) * 100 : 0
  ...
  return [
    ...
    },
    {
      ...
      value: formatCompactNumber(avgTokensPerMessage),
    ...
    },
    {
      ...
      hint: t('pages.dashboard.kpis.cacheReadRatio.hint', { cacheRead: formatCompactNumber(totals.cacheRead), input: formatCompactNumber(cacheInputTotal) }),
    },
    {
      key: 'active-days',
...
const topModels = computed(() => {
  const raw = sessionsUsageResult.value?.aggregates?.byModel
...
const topProviders = computed(() => {
  const raw = sessionsUsageResult.value?.aggregates?.byProvider
  const list = Array.isArray(raw) ? raw : []
  const key = usageMode.value === 'tokens' ? 'totalTokens' : 'totalCost'
  return [...list]
    .sort((a, b) => (b.totals[key] || 0) - (a.totals[key] || 0))
...
const topTools = computed(() => {
  const raw = sessionsUsageResult.value?.aggregates?.tools?.tools
  const list = Array.isArray(raw) ? raw : []
...
onMounted(async () => {
  ...
  prevWsState = wsStore.state

  cleanupStateChange = wsStore.subscribe('stateChange', () => {
    handleWsStateChange()
  })

  // Auto-refresh when sessions or crons change (create/delete/expire)
  cleanupSSE.push(
    wsStore.subscribe('event:session.new', () => { void refreshDashboard() }),
    wsStore.subscribe('event:session.end', () => { void refreshDashboard() }),
    wsStore.subscribe('event:cron.run', () => { void refreshDashboard() }),
    wsStore.subscribe('event:config.changed', () => { void refreshDashboard() }),
  )
...
function handleWsStateChange() {
  const current = wsStore.state
  if (current === 'connected' && prevWsState !== 'connected') {
    if (refreshing.value) {
...
async function refreshDashboard() {
  ...
  refreshedStartDate = usageStartDate.value
  refreshedEndDate = usageEndDate.value
  ...
  try {
    ...
    const datesDrifted = refreshedStartDate !== usageStartDate.value || refreshedEndDate !== usageEndDate.value
    if (pendingRefreshOnConnect || datesDrifted) {
...
function handleDateRangeChanged() {
  ...
  if (usageStartDate.value && usageEndDate.value && usageStartDate.value <= usageEndDate.value) {
    void refreshDashboard()
  }
}
    ...
    <div class="dashboard-page">
      <NCard class="dashboard-hero" :bordered="false">
        ...
        <NSpace :size="8" wrap class="dashboard-filters-row">
          ...
          <NButton size="small" :type="usageMode === 'cost' ? 'primary' : 'default'" secondary @click="usageMode = 'cost'">{{ t('pages.dashboard.usageMode.cost') }}</NButton>
        ...
        <NAlert v-if="usageError && wsStore.state === 'connected'" type="warning" :bordered="false" style="margin-top: 10px;">
      ...
      <NGrid cols="1 l:3" responsive="screen" :x-gap="12" :y-gap="12">
        ...
        <NGridItem :span="1" class="usage-structure-item">
          <NCard :title="t('pages.dashboard.cards.structure')" class="dashboard-card usage-structure-card">
            ...
            <NText v-if="usageTotals.missingCostEntries > 0" depth="3" style="display: block; margin-top: 8px; font-size: 12px;">
          ...
          <NGridItem>
            <div class="top-pane-card">
              ...
              <div v-if="topProviders.length" class="top-list">
                <div v-for="(item, index) in topProviders" :key="`provider-${index}`" class="top-row">
                  <div class="top-row-main">
          ...
          <NGridItem>
            <div class="top-pane-card">
              ...
              <div v-if="topTools.length" class="top-list">
                <div v-for="(item, index) in topTools" :key="`tool-${index}`" class="top-row">
                  <div class="top-row-main">
                    <span>{{ item.name }}</span>
